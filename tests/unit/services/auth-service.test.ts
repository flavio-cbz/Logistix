...
describe('AuthService', () => {
  ...
  describe('destroySession', () => {
    it('should destroy session successfully', async () => {
      ...
      // Assert
      expect(mockDb.queryOne).toHaveBeenCalledWith(
        expect.stringContaining('SELECT user_id FROM sessions WHERE id = ?'),
        [sessionId],
      ...
      expect(mockCookies.delete).toHaveBeenCalledWith('test_session');
    });
  ...
  const result = await authService.authenticate(credentials.username, credentials.password);
      ...
      expect(result.sessionId).toBe('test-uuid-1234-5678-9012');
