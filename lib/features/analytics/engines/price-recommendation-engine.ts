...
export class PriceRecommendationEngine {
  ...
  generateRecommendations(
    ...
    recommendations.push(
      this.generateCompetitiveStrategy(metrics, fullContext),
    );
    ...
    recommendations.push(
      this.generatePenetrationStrategy(metrics, fullContext),
    );

    // Trier par confiance et pertinence
    return recommendations.sort(
      (a, b) =>
        this.calculateRecommendationScore(b, fullContext) -
        this.calculateRecommendationScore(a, fullContext),
    );
  }
  ...
  private generateCompetitiveStrategy(
    ...
    return {
      ...
      competitivePosition: {
        ...
        vsAverage:
          (basePrice - metrics.descriptiveStats.mean) /
          metrics.descriptiveStats.mean,
    ...
    return {
      ...
      competitivePosition: {
        ...
        vsAverage:
          ...
          metrics.descriptiveStats.mean,
        vsMedian:
          (basePrice - metrics.descriptiveStats.median) /
          metrics.descriptiveStats.median,
      },
  ...
  private estimateSaleTime(
    ...
    if (
      context.itemCondition === "new" ||
      context.itemCondition === "excellent"
    ) {
      baseDays *= 0.8;
  ...
  private calculatePercentile(price: number, metrics: AdvancedMetrics): number {
    ...
    for (const bin of histogram) {
      if (price >= bin.range[0] && price <= bin.range[1]) {
        ...
        const binPosition =
          (price - bin.range[0]) / (bin.range[1] - bin.range[0]);
  ...
  private calculateRecommendationScore(
    ...
    if (
      context.userGoals.priority === "speed" &&
      recommendation.expectedSaleTime < 14
    ) {
      score += 20;
    } else if (
      context.userGoals.priority === "profit" &&
      recommendation.strategy === "premium"
    ) {
      score += 15;
    } else if (
      context.userGoals.priority === "market_share" &&
      recommendation.marketShare > 0.15
    ) {
      score += 15;
    }
    ...
    if (
      context.userGoals.riskTolerance === "conservative" &&
      recommendation.riskAssessment.level === "high"
    ) {
      score -= 30;
    } else if (
      context.userGoals.riskTolerance === "aggressive" &&
      recommendation.riskAssessment.level === "low"
    ) {
      score += 10;
    }
   ...
   */
  private generateCompetitiveReasoning(
    price: number,
    metrics: AdvancedMetrics,
    context: RecommendationContext,
  ): string[] {
    ...
    reasoning.push(
      `Prix aligné sur la médiane du marché (${median.toFixed(2)}€)`,
    );
    reasoning.push(
      `Positionnement au ${(percentile * 100).toFixed(0)}e percentile`,
    );
    ...
    reasoning.push(
      "Stratégie équilibrée entre vitesse de vente et rentabilité",
    );
   ...
   */
  private generatePremiumReasoning(
    price: number,
    metrics: AdvancedMetrics,
    context: RecommendationContext,
  ): string[] {
  ...
  private generateValueReasoning(
    ...
    if (
      context.inventory.urgency === "medium" ||
      context.inventory.urgency === "high"
    ) {
   ...
   */
  private generatePenetrationReasoning(
    price: number,
    metrics: AdvancedMetrics,
    context: RecommendationContext,
  ): string[] {
    const reasoning: string[] = [];
    const p10 =
      metrics.priceDistribution.percentiles[10] ||
      metrics.descriptiveStats.quartiles[0] * 0.8;

    // Inclure le prix recommandé pour éviter variable non utilisée
    reasoning.push(`Prix recommandé: ${price.toFixed(2)}€`);
    reasoning.push(
      `Prix très compétitif proche du 10e percentile (${p10.toFixed(2)}€)`,
    );
