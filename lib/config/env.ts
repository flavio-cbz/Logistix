...
const envSchema = z.object({
    ...
    SIGNUP_ENABLED: z.coerce.boolean().default(false),
    REQUIRE_INVITATION_CODE: z.coerce.boolean().default(false),
    INVITATION_CODES: z.string().optional(), // Comma-separated list
    MAX_SIGNUPS_PER_HOUR: z.coerce.number().default(10),

    // Superbuy Integration
    SUPERBUY_API_KEY: z.string().optional(),
    SUPERBUY_API_SECRET: z.string().optional(),
    SUPERBUY_CAPTCHA_DEBUG: z.coerce.boolean().default(false),
    CAPTCHA_DEBUG: z.coerce.boolean().default(false),
    ALLOW_HEADFUL: z.coerce.boolean().default(false),
...
export function getSignupConfig() {
    const e = getEnv();
    return {
        enabled: e.SIGNUP_ENABLED,
        requireInvitationCode: e.REQUIRE_INVITATION_CODE,
        validInvitationCodes: e.INVITATION_CODES?.split(',').filter(Boolean) || [],
        maxSignupsPerHour: e.MAX_SIGNUPS_PER_HOUR,
    };
}
