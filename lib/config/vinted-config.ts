...
export function getVintedConfig(): VintedConfig {
  return {
    ...
    autoRefreshEnabled:
      (process.env as any)["VINTED_AUTO_REFRESH_ENABLED"] === "false"
        ? false
        : VINTED_CONFIG.autoRefreshEnabled,
    refreshIntervalMinutes: process.env[
      "VINTED_TOKEN_REFRESH_INTERVAL_MINUTES"
    ]!
      ? parseInt(process.env["VINTED_TOKEN_REFRESH_INTERVAL_MINUTES"]!)
      : VINTED_CONFIG.refreshIntervalMinutes,
    proactiveRefreshMarginMinutes: process.env[
      "VINTED_PROACTIVE_REFRESH_MARGIN_MINUTES"
    ]!
