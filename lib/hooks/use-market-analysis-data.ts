...
export function useMarketAnalysisData() {
  const { historicalData, setHistoricalData, setPagination } =
  ...
  const loadHistoricalData = async (page = 1) => {
    ...
    try {
      const response = await autoPerf.autoFetch(
        `/api/v1/market-analysis?page=${page}&limit=10`
      );
      if (!response.ok) {
      ...
      setHistoricalData(
        page === 1
          ? transformedAnalyses
          : [...historicalData, ...transformedAnalyses],
      );
      setPagination({
        page: data.page || 1,
        limit: data.limit || 10,
