...
export function transformDbParcelleToEntity(dbParcelle: any): Parcelle {
  ...
  // Handle both snake_case (from raw SQL) and camelCase (from Drizzle ORM)
  const id = dbParcelle.id;
  const userId = dbParcelle.userId || dbParcelle.user_id;
  const numero = dbParcelle.numero;
  // const numeroSuivi = dbParcelle.numero_suivi; // Add numero_suivi support
  const transporteur = dbParcelle.transporteur;
  const nom = dbParcelle.nom;
  const statut = dbParcelle.statut;
  const actif = dbParcelle.actif ?? 1;
  const prixAchat = dbParcelle.prixAchat || dbParcelle.prix_achat;
  const poids = dbParcelle.poids;
  const prixTotal = dbParcelle.prixTotal || dbParcelle.prix_total;
  const prixParGramme = dbParcelle.prixParGramme || dbParcelle.prix_par_gramme;
  const createdAt = dbParcelle.createdAt || dbParcelle.created_at;
  const updatedAt = dbParcelle.updatedAt || dbParcelle.updated_at;
...
export function transformParcelleFormToUpdateInput(
  ...
  // Recalculate derived fields if base values are provided and not null
  if (transformed.prixAchat != null && transformed.poids != null && transformed.poids > 0) {
