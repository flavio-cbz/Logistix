...
<<<<<<< HEAD
    ...
    // Vinted statistics (unlocked when product is linked to Vinted)
    vintedStats: text("vinted_stats", { mode: "json" }).$type<{
      viewCount: number;
      favouriteCount: number;
      isReserved: boolean;
      isClosed: boolean;
      interestRate: number; // favouriteCount / viewCount * 100
      serviceFee?: number; // Vinted commission in EUR
...
// Vinted Sessions table - specific handling for Vinted auth
export const vintedSessions = sqliteTable(
  "vinted_sessions",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    sessionCookie: text("session_cookie").notNull(), // Encrypted
    status: text("status", { enum: ["active", "expired", "requires_refresh", "requires_configuration"] })
      .notNull()
