...
export interface ParcelleFilterOptions extends FilterOptions {
  ...
  prixAchatMin?: number | undefined;
  prixAchatMax?: number | undefined;
  poidsMin?: number | undefined;
  poidsMax?: number | undefined;
  prixTotalMin?: number | undefined;
  prixTotalMax?: number | undefined;
...
> {
  ...
  /**
   * Find parcelles by a list of numeros for a specific user
   */
  async findByNumeros(userId: string, numeros: string[]): Promise<Parcelle[]> {
      ...
      const results = await db
        ...
        .where(
          and(
            ...
            inArray(parcelles.numero, numeros)
          )
        );

      return results.map(transformDbParcelleToEntity);
    });
  }
  ...
  /**
   * Get transporteur suggestions for search
   */
  public async getTransporteurSuggestions(query: string): Promise<{ transporteur: string; count: number }[]> {
    try {
      return await this.executeCustomQuery((db) => {
        const results = db
          .select({
            transporteur: this.table.transporteur,
            count: sql<number>`COUNT(*)`,
          })
          .from(this.table)
          .where(sql`${this.table.transporteur} LIKE ${`%${query}%`}`)
          .groupBy(this.table.transporteur)
      ...
      }, "getTransporteurSuggestions");
    } catch (error) {
      this.logError("getTransporteurSuggestions failed", error, { query });
      return [];
    }
  }
}
