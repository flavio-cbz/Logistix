 ...
 */
export function createChartDescription(
  _data: any[],
  type: "line" | "bar" | "pie" | "area" = "line",
): string {
  ...
  return (
    `Graphique ${type} avec ${dataPoints} points de données. ` +
...
export function createStatsDescription(
  ...
  if (trend) {
    const trendDirection =
      trend.value > 0 ? "en hausse" : trend.value < 0 ? "en baisse" : "stable";
 ...
 */
export function createLoadingDescription(
  component: string,
  progress?: number,
): string {
