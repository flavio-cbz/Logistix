...
export class QueueManager {
  ...
  private constructor(concurrencyLimit: number = 5) {
    ...
    logger.info(`QueueManager initialized with concurrency limit: ${concurrencyLimit}`);
  }
  ...
  public static getInstance(concurrencyLimit?: number): QueueManager {
    if (!QueueManager.instance) {
      ...
      logger.info(`QueueManager concurrency limit updated to: ${concurrencyLimit}`);
    }
  ...
  public enqueue(
    ...
    return new Promise((resolve, reject) => {
      const item: QueueItem = { type, priority, timeout, context, resolve, reject, enqueuedAt: Date.now() };
      this.queue.push(item);
      logger.debug(`Operation "${context}" enqueued. Current queue size: ${this.queue.length}`);
      this.processQueue();
    });
  }
  ...
  public async processQueue(connection?: Database.Database): Promise<void> {
    logger.debug(`Processing queue. Running: ${this.runningCount}, Queue size: ${this.queue.length}, Limit: ${this.concurrencyLimit}`);
    // Si une connexion est passée, l'utiliser pour la prochaine requête en attente
    if (connection) {
      const item = this.queue.shift();
      if (item) {
        item.resolve(connection);
        logger.debug(`Operation "${item.context}" resolved with provided connection.`);
      }
    }
    ...
    while (this.runningCount < this.concurrencyLimit && this.queue.length > 0) {
      ...
      if (item) {
        this.runningCount++;
        logger.debug(`Starting operation "${item.context}". Running: ${this.runningCount}`);
        try {
          ...
          setTimeout(() => {
            if (!this.isShuttingDown) {
              item.resolve(db);
              logger.debug(`Operation "${item.context}" completed successfully.`);
            } else {
          ...
          item.reject(error);
          logger.error(`Operation "${item.context}" failed.`, { error: error instanceof Error ? error.message : String(error) });
        } finally {
          this.runningCount--;
          logger.debug(`Operation "${item.context}" finished. Running: ${this.runningCount}`);
  ...
  public clearQueue(): void {
    ...
    clearedItems.forEach(item => {
      ...
      logger.debug(`Operation "${item.context}" cancelled due to queue clear.`);
    });
  ...
  public setConcurrencyLimit(newLimit: number): void {
    if (newLimit <= 0) {
      logger.warn(`Attempted to set invalid concurrency limit: ${newLimit}. Must be greater than 0.`);
      return;
    }
  ...
  public getQueueByType(): {
    ...
    this.queue.forEach(item => {
      ...
      stats[item.type]![item.priority] = (stats[item.type]![item.priority] || 0) + 1;
    });
    return stats;
  }
}
