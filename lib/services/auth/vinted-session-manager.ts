...
class VintedSessionManager {
  ...
  async getSessionCookie(userId: string): Promise<string | null> {
    try {
      ...
      try {
        const decrypted = await decryptSecret(session.session_cookie);
        return decrypted;
      } catch (error) {
        logger.error('Failed to decrypt session cookie', {
          userId,
  ...
  async refreshSession(userId: string): Promise<RefreshResponse> {
    try {
      ...
      try {
        const cookie = await decryptSecret(session.session_cookie);
        ...
        const encryptedCookie = await encryptSecret(updatedCookie);
