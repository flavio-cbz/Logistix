...
export type AuthInstrumentationEventType = typeof AuthInstrumentationEventTypes[number];
...
export interface AuthInstrumentationMetrics {
  ...
  eventsByTimeframe: Record<'hour' | 'day' | 'week' | 'month', Record<AuthInstrumentationEventType, number>>;
  errorsByTimeframe: Record<'hour' | 'day' | 'week' | 'month', Record<AuthInstrumentationEventType, number>>;
  ...
  metricsByProvider: Record<string, { totalEvents: number; successfulEvents: number; failedEvents: number; totalLatency: number; }>;
  metricsByModel: Record<string, { totalEvents: number; successfulEvents: number; failedEvents: number; totalLatency: number; }>;
}
...
export class AuthInstrumentationCollector {
  ...
  public static getInstance(): AuthInstrumentationCollector {
    if (!AuthInstrumentationCollector.instance) {
      AuthInstrumentationCollector.instance = new AuthInstrumentationCollector();
    }
  ...
  private initializeMetrics(): AuthInstrumentationMetrics {
    ...
    AuthInstrumentationEventTypes.forEach(type => {
      ...
      Object.keys(initialMetrics.eventsByTimeframe).forEach(tf => {
        initialMetrics.eventsByTimeframe[tf as 'hour' | 'day' | 'week' | 'month'][type] = 0;
        initialMetrics.errorsByTimeframe[tf as 'hour' | 'day' | 'week' | 'month'][type] = 0;
      });
    });
  ...
  recordEvent(event: AuthInstrumentationEvent): void {
    try {
      ...
      logger.debug('[AuthInstrumentation] Event recorded', { event });
    } catch (error) {
      logger.error('[AuthInstrumentation] Error recording event', { error: getErrorMessage(error), event });
    }
  }
  ...
  private updateMetrics(event: AuthInstrumentationEvent): void {
    ...
    if (event.latency !== undefined) {
      const currentLatencySum = (this.metrics.averageLatency[event.type] || 0) * (this.metrics.eventsByType[event.type] - 1);
      this.metrics.averageLatency[event.type] = (currentLatencySum + event.latency) / this.metrics.eventsByType[event.type];
    ...
    const timeframeKeys: Array<'hour' | 'day' | 'week' | 'month'> = ['hour', 'day', 'week', 'month'];
    timeframeKeys.forEach(tf => {
      const tfMetrics = this.metrics.eventsByTimeframe[tf];
      tfMetrics[event.type] = (tfMetrics[event.type] || 0) + 1;
      if (!event.success) {
        this.metrics.errorsByTimeframe[tf][event.type] = (this.metrics.errorsByTimeframe[tf][event.type] || 0) + 1;
      }
    });
    ...
    if (event.origin) {
      if (!this.metrics.eventsByOrigin[event.origin]) {
        this.metrics.eventsByOrigin[event.origin] = {} as Record<AuthInstrumentationEventType, number>;
        this.metrics.errorsByOrigin[event.origin] = {} as Record<AuthInstrumentationEventType, number>;
      }
      this.metrics.eventsByOrigin[event.origin][event.type] = (this.metrics.eventsByOrigin[event.origin][event.type] || 0) + 1;
      if (!event.success) {
        this.metrics.errorsByOrigin[event.origin][event.type] = (this.metrics.errorsByOrigin[event.origin][event.type] || 0) + 1;
      }
    }
    ...
    if (event.provider) {
      if (!this.metrics.metricsByProvider[event.provider]) {
        this.metrics.metricsByProvider[event.provider] = { totalEvents: 0, successfulEvents: 0, failedEvents: 0, totalLatency: 0 };
      }
    ...
    if (event.model) {
      if (!this.metrics.metricsByModel[event.model]) {
        this.metrics.metricsByModel[event.model] = { totalEvents: 0, successfulEvents: 0, failedEvents: 0, totalLatency: 0 };
      }
  ...
  private calculateRates(): void {
    ...
    this.metrics.loginSuccessRate = totalLoginAttempts > 0 ? successfulLogins / totalLoginAttempts : 0;

    const totalSessionEvents = (this.metrics.eventsByType.session_create || 0) + (this.metrics.eventsByType.session_renew || 0);
    const successfulSessionRenewals = this.metrics.eventsByType.session_renew || 0;
    this.metrics.sessionRenewalRate = totalSessionEvents > 0 ? successfulSessionRenewals / totalSessionEvents : 0;
  }
  ...
  private cleanupOldEvents(): void {
    ...
    if (cleanedCount > 0) {
      logger.info('[AuthInstrumentation] Cleaned up old events', { cleanedCount });
    }
  }
  ...
  importEvents(events: AuthInstrumentationEvent[]): void {
    ...
    logger.info('[AuthInstrumentation] Events imported and metrics re-calculated.', { count: events.length });
  }
}

// Instance singleton
export const authInstrumentationCollector = AuthInstrumentationCollector.getInstance();