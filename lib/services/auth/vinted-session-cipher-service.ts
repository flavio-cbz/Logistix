...
import { deriveUserKek, generateUserSecret } from '@/lib/services/security/encryption-service';
...
async function deriveKekForUser(userId: string): Promise<Buffer> {
  try {
    ...
    if (row?.encryption_secret) {
      ...
      logger.info(`[deriveKekForUser] Nouveau encryption_secret généré et sauvegardé pour userId=${userId}`);
      return await deriveUserKek(newSecret);
    }
  } catch (err: unknown) { // Utilisation de unknown pour une meilleure gestion des erreurs
    ...
    if (msg.includes('no such column') || msg.includes('no such table') || msg.includes('SQLITE_ERROR')) {
      throw new Error('[deriveKekForUser] La colonne encryption_secret est manquante dans la table users. Migration requise.');
    } else {
      throw err;
    }
  }
...
function decodeParts(str: string): { iv: Buffer; tag: Buffer; encrypted: Buffer } {
...
class VintedSessionCipherService {
   ...
   */
  public async encryptSession(userId: string, plainToken: string): Promise<{ encryptedToken: string; encryptedDek: string; metadata: string }> {
    // Validation et logs d'entrée
    logger.debug("[encryptSession] called", {
      userId_type: typeof userId,
      plainToken_type: typeof plainToken,
      userId_valid: typeof userId === "string" && userId.length > 0,
      plainToken_valid: typeof plainToken === "string" && plainToken.length > 0,
      plainToken_length: typeof plainToken === "string" ? plainToken.length : undefined,
      plainToken_preview: typeof plainToken === "string" ? plainToken.slice(0, 6) + "***" : undefined,
    });
    ...
    if (typeof plainToken !== "string" || plainToken.length === 0) {
      logger.error("[encryptSession] plainToken invalide", { plainToken_length: plainToken?.length });
      throw new Error("plainToken must be a non-empty string");
    }

    // 1. generate DEK
    const dek = randomBytes(32);
    logger.debug("[encryptSession] DEK generated", { dek_length: dek.length, dek_preview: dek.toString("base64").slice(0, 16) });

    // 2. encrypt token with DEK
    const iv = randomBytes(IV_LENGTH);
    logger.debug("[encryptSession] Token IV generated", { iv_length: iv.length, iv_base64: iv.toString("base64") });

    const cipher = createCipheriv(ALGORITHM, dek, iv);
    const encryptedTokenBuf = Buffer.concat([cipher.update(plainToken, 'utf8'), cipher.final()]);
    const tag = cipher.getAuthTag();
    logger.debug("[encryptSession] Token tag generated", { tag_length: tag.length, tag_base64: tag.toString("base64") });
    ...
    logger.debug("[encryptSession] KEK derived", { kek_length: kek.length, kek_preview: kek.toString("base64").slice(0, 16) });

    const dekIv = randomBytes(IV_LENGTH);
    logger.debug("[encryptSession] DEK IV generated", { dekIv_length: dekIv.length, dekIv_base64: dekIv.toString("base64") });
    ...
    logger.debug("[encryptSession] DEK tag generated", { dekTag_length: dekTag.length, dekTag_base64: dekTag.toString("base64") });
    ...
    logger.debug("[encryptSession] output ready", {
   ...
   */
  public async decryptSession(userId: string, encryptedToken: string, encryptedDek?: string | null): Promise<string> {
    if (!encryptedDek) {
      throw new Error('No encrypted DEK present for this session. Legacy decryption n’est plus supportée.');
    }

    // decrypt DEK with per-user KEK
    const kek = await this.getKek(userId);
    const { iv: dekIv, tag: dekTag, encrypted: dekEncrypted } = decodeParts(encryptedDek);
