...
export class ModelDiscoveryService {
  ...
  public detectEndpointType(url: string): EndpointInfo {
    ...
    if (normalizedUrl.includes('openai.com') || normalizedUrl.includes('/v1/chat/completions')) {
      return {
        url,
        type: 'openai',
        requiresAuth: true,
    ...
    if (normalizedUrl.includes('generativelanguage.googleapis.com') || normalizedUrl.includes('gemini')) {
      return {
        url,
        type: 'gemini',
        requiresAuth: true,
    ...
    if (normalizedUrl.includes('nvidia.com') || normalizedUrl.includes('integrate.api.nvidia.com')) {
      return {
        url,
        type: 'nvidia',
        requiresAuth: true,
    ...
    if (normalizedUrl.includes('anthropic.com') || normalizedUrl.includes('claude')) {
      return {
        url,
        type: 'anthropic',
        requiresAuth: true
   ...
   */
  public async discoverModels(endpointUrl: string, apiKey: string): Promise<ModelRecommendation[]> {
    ...
    try {
      ...
      } else if (endpointInfo.type === 'openai' || endpointInfo.type === 'nvidia' || endpointInfo.type === 'custom') {
        availableModels = await this.discoverOpenAICompatibleModels(endpointInfo.modelsEndpoint!, apiKey);
      }
   ...
   */
  private async discoverGeminiModels(endpointUrl: string, apiKey: string): Promise<string[]> {
   ...
   */
  private async discoverOpenAICompatibleModels(modelsEndpoint: string, apiKey: string): Promise<string[]> {
    try {
   ...
   */
  private classifyDiscoveredModels(modelIds: string[], providerType: string): ModelRecommendation[] {
   ...
   */
  private createHeuristicRecommendation(modelId: string, providerType: string): ModelRecommendation {
  ...
  public getTopModelsByCategory(): Record<string, ModelRecommendation[]> {
    ...
    return {
      champions: allModels.filter(m => m.category === 'champion').sort((a, b) => b.score - a.score),
      excellent: allModels.filter(m => m.category === 'excellent').sort((a, b) => b.score - a.score),
      fast: allModels.filter(m => m.category === 'fast').sort((a, b) => b.efficiency - a.efficiency),
      good: allModels.filter(m => m.category === 'good').sort((a, b) => b.score - a.score),
      budget: allModels.filter(m => m.category === 'budget').sort((a, b) => b.efficiency - a.efficiency)
    };
  }
}