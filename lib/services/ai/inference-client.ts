...
export async function runInference(payload: InferenceRequest): Promise<InferenceResponse> {
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), config.maxProcessingTime);
    ...
    if (!response.ok) {
      ...
      throw createErrorFromResponse(errorBody, { payload, statusCode: response.status });
    }
...
export async function runMarketAnalysisInference(
  ...
  const enhancedPayload = {
    ...
    temperature: payload.temperature || getOptimalTemperature(payload.analysisType),
    max_tokens: payload.max_tokens || getOptimalMaxTokens(payload.analysisType),
  };

  try {
    const result = await runInference(enhancedPayload) as MarketAnalysisInferenceResponse;
