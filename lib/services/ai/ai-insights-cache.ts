...
import type { AIInsights, AIRecommendations, TrendPrediction, AnomalyDetection, VintedAnalysisResult } from '@/types/vinted-market-analysis';
...
export class AIInsightsCacheService {
  ...
  async cacheInsights(
    ...
    try {
      ...
      logger.error('[AIInsightsCache] Erreur lors de la mise en cache des insights', {
        ...
        analysisDataSize: Array.isArray(analysisData?.items) ? analysisData.items.length : 0,
      });
    }
  }
  ...
  async cacheRecommendations(
    ...
    try {
      ...
      const cacheTTL = this.normalizeTTL(ttl ?? this.getCacheTTL('recommendations'));
  ...
  private async invalidateByType(
    ...
    try {
      ...
      this.metrics.errors++;
      logger.error(`[AIInsightsCache] Erreur lors de l'invalidation (${type})`, {
        error: getErrorMessage(error),
      });
    }
  }
  ...
  getMetrics(): CacheMetrics {
    ...
    const hitRate = Math.round(Math.max(0, Math.min(1, hitRateRaw)) * 100) / 100;
  ...
  private getCacheTTL(type: AnalysisType): number {
    ...
    const baseTTL =
      typeof (perf as any)?.cacheExpiry === 'number' && isFinite((perf as any).cacheExpiry)
