...
class CacheManager {
  ...
  private cache: Map<string, { data: unknown; timestamp: number; ttl: number }> = new Map(); // `any` remplacé par `unknown`
   ...
   */
  private generateCacheKey(key: Record<string, unknown>): string { // `any` remplacé par `unknown`
   ...
   */
  async get<T>(key: Record<string, unknown>): Promise<T | null> { // `any` remplacé par `unknown`
   ...
   */
  async set<T>(key: Record<string, unknown>, data: T, _operationId?: string, ttl?: number): Promise<void> { // `any` remplacé par `unknown`
   ...
   */
  async delete(key: Record<string, unknown>): Promise<boolean> { // `any` remplacé par `unknown`
    const cacheKey = this.generateCacheKey(key);
