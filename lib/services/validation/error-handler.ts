...
export interface FallbackConfig<T = unknown> {
  // T pour le type de fallbackValue et le retour de fallbackFunction
  enabled: boolean;
...
export class ErrorHandler {
  ...
  public async executeWithRecovery<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        return {
          ...
          attemptsUsed,
          recoveryStrategy:
            attempt > 1 ? RecoveryStrategy.RETRY : RecoveryStrategy.RETRY, // Peut-être toujours RETRY ici?
          degradedMode: this.degradedMode,
        };
      } catch (error: unknown) {
        // Typage de l'erreur du catch
        ...
        if (attempt < maxAttempts && this.shouldRetry(categorizedError)) {
          ...
          logger.info(
            `Retrying operation ${operationName} in ${delay}ms (attempt ${attempt + 1}/${maxAttempts})`,
          );
    ...
    if (lastError) {
      return await this.handleFailedOperation(
        lastError,
        operationName,
        fallbackConfig,
        attemptsUsed,
  ...
  private async handleFailedOperation<T>(
    ...
    switch (error.recoveryStrategy) {
      ...
      case RecoveryStrategy.DEGRADE:
        return await this.executeDegradedMode(
          error,
          operationName,
          fallbackConfig,
          attemptsUsed,
      ...
      default:
        logger.error(
          `Aborting due to critical error in operation: ${operationName}`,
        );
        return {
          success: false,
          error,
  ...
  private async executeFallback<T>(
    ...
    try {
      ...
      if (fallbackConfig.degradedMode) {
        ...
        logger.warn(
          `Entering degraded mode due to fallback in operation: ${operationName}`,
        );
      }
  ...
  private shouldRetry(error: CategorizedError): boolean {
    ...
    if (recentSimilarErrors > 10) {
      logger.warn(
        `Too many similar errors recently, skipping retry for category: ${error.category}`,
      );
      return false;
    }
  ...
  private isTokenError(message: string, name: string): boolean {
    ...
    return (
      tokenPatterns.some((pattern) => pattern.test(message)) ||
  ...
  private isNetworkError(message: string, name: string): boolean {
    ...
    return (
      networkPatterns.some((pattern) => pattern.test(message)) ||
  ...
  private isTimeoutError(message: string, name: string): boolean {
    ...
    return (
      timeoutPatterns.some((pattern) => pattern.test(message)) ||
      name === "TimeoutError" ||
      name === "AbortError"
    );
  }
  ...
  private isApiError(
    ...
    return (
      apiPatterns.some((pattern) => pattern.test(message)) ||
      hasApiContext ||
      name === "HTTPError"
    );
  }
  ...
  private isDatabaseError(message: string, name: string): boolean {
    ...
    return (
      dbPatterns.some((pattern) => pattern.test(message)) ||
      name === "DatabaseError" ||
      name === "SQLError"
    );
  }
  ...
  private isValidationError(message: string, name: string): boolean {
    ...
    return (
      validationPatterns.some((pattern) => pattern.test(message)) ||
  ...
  private isConfigurationError(message: string, name: string): boolean {
    ...
    return (
      configPatterns.some((pattern) => pattern.test(message)) ||
  ...
  private getApiErrorSeverity(
    ...
    if (
      context?.["message"] &&
      (context["message"] as string).includes("rate limit")
    )
      return ErrorSeverity.MEDIUM;
    if (
      context?.["message"] &&
      (context["message"] as string).includes("quota exceeded")
    )
  ...
  private getApiRecoveryStrategy(
    context?: Record<string, unknown>,
  ): RecoveryStrategy {
    // Removed message parameter
    const statusCode =
  ...
  private isApiErrorRetryable(context?: Record<string, unknown>): boolean {
    // Removed message parameter
    const statusCode =
      typeof context?.["statusCode"] === "number"
        ? context["statusCode"]
