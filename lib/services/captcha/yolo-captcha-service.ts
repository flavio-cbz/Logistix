...
export class YoloCaptchaService {
  ...
  public async detect(imagePath: string): Promise<YoloCaptchaResult> {
    ...
    // console.log('[YoloCaptchaService] Max confidence per class:', maxConfPerClass.map((c, i) => `Class ${i}: ${(c * 100).toFixed(1)}%`).join(', '));
  ...
  public async planSliderMovement(
    ...
    if (!movingPiece || !targetHole) {
      // console.log('[YoloCaptchaService] Explicit class matching failed, falling back to spatial heuristics');
