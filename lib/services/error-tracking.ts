...
export class ErrorTrackingService {
  ...
  public captureException(
    ...
    if (!this.isInitialized) {
      // Fallback logging in development/uninitialized state
      logger.warn(
        "ErrorTrackingService not initialized. Exception captured locally:",
  ...
  public captureMessage(
    ...
    if (!this.isInitialized) {
      // Fallback logging
      logger.warn(
        `ErrorTrackingService not initialized. Message (${level}) captured locally: ${message}`,
        { ...context },
      );
      return;
    }
