...
export class ErrorTrackingService {
  ...
  public initialize(config: ErrorTrackingConfig): void {
    ...
    if (config.dsn && process.env.NEXT_PUBLIC_SENTRY_DSN) {
      ...
      logger.warn('ErrorTrackingService not initialized: Sentry DSN not provided or NEXT_PUBLIC_SENTRY_DSN missing.');
    }
  }
   ...
   */
  public captureException(error: unknown, context?: Record<string, unknown>): void { // Changed error to unknown, metadata to Record<string, unknown>
    if (!this.isInitialized) {
      logger.warn('ErrorTrackingService not initialized. Cannot capture exception.', { error: error instanceof Error ? error.message : String(error) });
      return;
    }
   ...
   * @param context Contexte supplémentaire sous forme de clé-valeur.
   */
  public captureMessage(message: string, level: 'info' | 'warning' | 'error' = 'info', context?: Record<string, unknown>): void { // Changed metadata to Record<string, unknown>
    if (!this.isInitialized) {
      logger.warn('ErrorTrackingService not initialized. Cannot capture message.', { message, level });
      return;
    }
   ...
   */
  public addBreadcrumb(message: string, category?: string, level: 'info' | 'warning' | 'error' = 'info', metadata?: Record<string, unknown>): void { // Changed metadata to Record<string, unknown>
    if (!this.isInitialized) {
      logger.debug('ErrorTrackingService not initialized. Cannot add breadcrumb.', { message, category, level });
      return;
    }
    logger.debug('Breadcrumb:', message, { category, level, ...metadata });
  ...
  public setUser(user: { id?: string; email?: string; username?: string } | null): void {
    if (!this.isInitialized) {
      logger.warn('ErrorTrackingService not initialized. Cannot set user context.');
      return;
    }
    // Sentry.setUser(user);
  ...
  public clearUser(): void {
    if (!this.isInitialized) {
      logger.warn('ErrorTrackingService not initialized. Cannot clear user context.');
      return;
    }
    // Sentry.setUser(null);
  ...
  public setTags(tags: Record<string, string>): void {
    ...
    // Sentry.setTags(tags);
    logger.debug('Global tags set:', tags);
  }
   ...
   */
  public setExtras(extras: Record<string, unknown>): void { // Changed extras to Record<string, unknown>
    if (!this.isInitialized) {
      logger.warn('ErrorTrackingService not initialized. Cannot set extras.');
      return;
    }
    // Sentry.setExtras(extras);
    logger.debug('Global extras set:', extras);
  }
