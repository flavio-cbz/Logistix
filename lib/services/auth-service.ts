  ...
  async createSession(userId: string): Promise<string> {
    return this.executeOperation(
      ...
      async () => {
        ...
        await databaseService.execute(
          `INSERT INTO user_sessions (id, user_id, expires_at, created_at, last_activity_at) VALUES (?, ?, ?, ?, ?)`,
          [sessionId, userId, expiresAt, timestamp, timestamp],
          "createSession",
        );
  ...
  async revokeAllSessionsExcept(userId: string, currentSessionId: string): Promise<number> {
    return this.executeOperation(
      ...
      async () => {
        ...
        this.logger.info("Revoked other sessions", {
          userId,
  ...
  async revokeUserSession(userId: string, sessionId: string): Promise<void> {
    return this.executeOperation(
      ...
      async () => {
        ...
        const session = await databaseService.queryOne<{ id: string }>(
        ...
        if (!session) {
          throw this.createNotFoundError("Session", sessionId);
        }
        ...
        this.logger.info("Revoked user session", {
          userId,
          sessionId
        });
      },
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string): Promise<void> {
    return this.executeOperation(
      ...
      async () => {
        ...
        const user = await databaseService.queryOne<{ id: string; password_hash: string }>(
        ...
        const passwordMatches = await this.verifyPassword(currentPassword, user.password_hash);
        if (!passwordMatches) {
          throw this.createAuthError("Invalid current password");
        }
