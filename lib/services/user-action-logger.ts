...
class UserActionLoggerService {
  ...
  async logUserAction(
    ...
    if (context.sessionId !== undefined)
      auditContext.sessionId = context.sessionId;
    if (context.ip !== undefined) auditContext.ip = context.ip;
    if (context.userAgent !== undefined)
      auditContext.userAgent = context.userAgent;
    if (context.requestId !== undefined)
      auditContext.requestId = context.requestId;
  ...
  async logFailedUserAction(
    ...
    if (context.sessionId !== undefined)
      auditFailedContext.sessionId = context.sessionId;
    if (context.ip !== undefined) auditFailedContext.ip = context.ip;
    if (context.userAgent !== undefined)
      auditFailedContext.userAgent = context.userAgent;
    if (context.requestId !== undefined)
      auditFailedContext.requestId = context.requestId;
    ...
    if (context.userAgent !== undefined)
      errorContext.userAgent = context.userAgent;
    ...
    if (details.resourceId !== undefined)
      errorMeta.resourceId = details.resourceId;

    errorTrackingService.captureException(error, {
      ...errorContext,
      ...errorMeta,
    }); // Changed to use errorTrackingService
    ...
    this.logger.error(
      `User action failed: ${details.action} on ${details.resource}`,
      error,
      {
        actionId,
        userId: context.userId,
    ...
    await this.checkForSuspiciousActivity(context, {
      ...details,
      success: false,
    });
  }
  ...
  async logAuthenticationEvent(
    ...
    if (context.sessionId !== undefined)
      authContext.sessionId = context.sessionId;
    if (context.ip !== undefined) authContext.ip = context.ip;
    if (context.userAgent !== undefined)
      authContext.userAgent = context.userAgent;
    if (context.requestId !== undefined)
      authContext.requestId = context.requestId;
  ...
  async logDataAccessEvent(
    ...
    if (context.sessionId !== undefined)
      dataAccessContext.sessionId = context.sessionId;
    if (context.ip !== undefined) dataAccessContext.ip = context.ip;
    if (context.userAgent !== undefined)
      dataAccessContext.userAgent = context.userAgent;
    if (context.requestId !== undefined)
      dataAccessContext.requestId = context.requestId;
    ...
    await auditUserAction(
      context.userId,
      {
        ...
        details: {
          operation,
          ...(sanitizedDetails?.fields !== undefined
            ? { fields: sanitizedDetails.fields }
            : {}),
          ...(sanitizedDetails?.oldValues !== undefined
            ? { oldValues: sanitizedDetails.oldValues }
            : {}),
          ...(sanitizedDetails?.newValues !== undefined
            ? { newValues: sanitizedDetails.newValues }
            : {}),
          ...(sanitizedDetails?.query !== undefined
  ...
  getUserActivitySummary(
    ...
    userActions.forEach(({ context, details }) => {
      (actionsByType as any)[details.action] =
        (actionsByType[details.action]! || 0) + 1;
  ...
  private endUserSession(sessionId: string): void {
    ...
    if (session) {
      ...
      performanceMetrics.recordMetric("user_session_duration", duration, "ms", {
        userId: session.userId,
  ...
  private async checkForSuspiciousActivity(
    ...
    const recentUserActions = this.recentActions.filter(
      ({ context: ctx }) =>
    ...
    const recentFailures = recentUserActions.filter(
      ({ details: d }) => !d.success,
    );
  ...
  private sanitizeDataAccessDetails(details?: {
    ...
    if (details.query) {
      sanitized.query = details.query.replace(
        /password\s*=\s*'[^']*'/gi,
        "password = '[REDACTED]'",
      );
    } else {
...
export const logUserAction =
  userActionLogger.logUserAction.bind(userActionLogger);
export const logFailedUserAction =
  userActionLogger.logFailedUserAction.bind(userActionLogger);
export const logAuthenticationEvent =
  userActionLogger.logAuthenticationEvent.bind(userActionLogger);
export const logDataAccessEvent =
  userActionLogger.logDataAccessEvent.bind(userActionLogger);
