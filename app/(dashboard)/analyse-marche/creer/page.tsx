...
function AnalysisCreationDashboard(): JSX.Element {
  ...
  const canSubmit = Boolean(productName && productName.length >= 3 && typeof catalogId === "number");

  const submit = async (values: MarketAnalysisRequest) => {
    const success = await launchMarketAnalysis(values);
    if (success) {
      try {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Formulaire complet pour configurer et lancer une analyse</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
