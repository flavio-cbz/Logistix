...
function calculateOpportunityScore({ avgPrice, salesVolume }: OpportunityScoreInput): { score: number; label: string; icon: React.ReactNode; color: string } {
  if (!avgPrice.length || !salesVolume.length) {
    return { score: 0, label: 'Données insuffisantes', icon: <HelpCircle className="text-[hsl(var(--muted-foreground))] h-6 w-6" />, color: 'gray' };
  }
  ...
  if (score > 80) return { score, label: 'Excellente opportunité', icon: <Trophy className="text-[hsl(var(--warning-foreground))] h-6 w-6" />, color: 'yellow' };
  if (score > 40) return { score, label: 'Bonne opportunité', icon: <ThumbsUp className="text-[hsl(var(--success-foreground))] h-6 w-6" />, color: 'green' };
  if (score > 0) return { score, label: 'Opportunité limitée', icon: <ThumbsDown className="text-[hsl(var(--destructive-foreground))] h-6 w-6" />, color: 'red' };
  return { score, label: 'Données insuffisantes', icon: <HelpCircle className="text-[hsl(var(--muted-foreground))] h-6 w-6" />, color: 'gray' };
}
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex items-center gap-4 mb-2">
          {icon}
          <span className={`text-2xl font-bold ${color === 'yellow' ? 'text-[hsl(var(--warning-foreground))]' : color === 'green' ? 'text-[hsl(var(--success-foreground))]' : color === 'red' ? 'text-[hsl(var(--destructive-foreground))]' : 'text-[hsl(var(--muted-foreground))]'}`}>
            {score}
          </span>
        </div>
