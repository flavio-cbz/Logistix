...
import { ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react'; // Removed TrendingUp
...
const getTrendIcon = (trend: PricePrediction['trend']) => { // Removed !
  if (trend === 'up') return <ArrowUpRight className="text-[hsl(var(--success-foreground))] h-6 w-6" />;
  if (trend === 'down') return <ArrowDownRight className="text-[hsl(var(--destructive-foreground))] h-6 w-6" />;
  return <Minus className="text-[hsl(var(--muted-foreground))] h-6 w-6" />;
};

const getTrendLabel = (trend: PricePrediction['trend']) => { // Removed !
