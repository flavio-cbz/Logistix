          ...
          <TableBody>
            {topParcelles.map((parcelle) => (
              <TableRow key={parcelle.id}>
                ...
                <TableCell className="text-right">{(parcelle.prixTotal ?? 0).toFixed(2)}€</TableCell>
                <TableCell className="text-right">{(parcelle.beneficesTotal ?? 0).toFixed(2)}€</TableCell>
                <TableCell className="text-right">
                  <span className={parcelle.roi > 0 ? "text-[hsl(var(--success-foreground))]" : "text-[hsl(var(--destructive-foreground))]"}>
