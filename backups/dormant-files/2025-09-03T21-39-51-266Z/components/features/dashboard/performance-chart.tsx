...
export default function PerformanceChart({ produits }: PerformanceChartProps) {
  ...
  const CustomTooltip = ({ active, payload, label }: CustomTooltipProps) => {
    if (active && payload && payload.length) {
      return (
        <div
          className={`p-3 rounded-md shadow-md ${isDark ? "bg-[hsl(var(--muted))] text-[hsl(var(--primary-foreground))] bg-[hsl(var(--card))]" : "text-[hsl(var(--primary-foreground))] bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))]"} border ${isDark ? "border-[hsl(var(--border))]" : "border-[hsl(var(--border))]"}`}
        >
          <p className="font-medium">{label}</p>
          {payload.map((entry: any, _index: number) => (
            <p key={`item-${_index}`} style={{ color: entry.color }}>
