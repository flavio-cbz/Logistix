...
export default function TempsVente({ produits }: TempsVenteProps) {
  ...
  const distribution = produits.reduce(
    (acc, produit) => {
      ...
      const jours = Number.parseInt(produit.tempsEnLigne.split(" ")[0] || "0", 10) // Ajout de la base 10 pour parseInt
      
      let categorie: string;

      if (isNaN(jours)) { // Gérer le cas où jours n'est pas un nombre valide
        categorie = "Inconnu";
      } else if (jours >= 0 && jours <= 3) {
        categorie = "0-3 jours";
      } else if (jours > 3 && jours <= 7) {
        categorie = "4-7 jours";
      } else if (jours > 7 && jours <= 14) {
        categorie = "8-14 jours";
      } else if (jours > 14 && jours <= 30) {
        categorie = "15-30 jours";
      } else { // jours > 30
        categorie = "30+ jours";
      }
  ...
  const data = [
    ...
    { periode: "Inconnu", nombre: distribution["Inconnu"] || 0 }, // Ajouter la catégorie "Inconnu"
  ]
