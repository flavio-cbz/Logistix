...
export default function DashboardConfig({ config }: DashboardConfigProps) {
  ...
  const [cards, setCards] = useState<DashboardConfigType["cards"]>([]) // Correction: suppression de !
  ...
  const handleDragEnd = (result: any) => {
    ...
    const updatedCards = items.map((card, idx) => ({ // Correction: _index en idx
      ...card,
      order: idx, // Correction: index en idx
    }))
  ...
  const handleUpdateCard = (cardId: string, _data: Partial<DashboardCard>) => { // Correction: data en _data
    setCards(cards.map((card) => (card.id === cardId ? { ...card, ..._data } : card))) // Correction: data en _data
  }
  ...
  const moveCard = (cardId: string, direction: "up" | "down") => {
    ...
    if (index === -1) return // Correction: _index en index
    ...
    if (direction === "up") {
      newCards[index] = newCards[index - 1]! // Correction: (newCards as any)[index]
      newCards[index - 1] = card! // Correction: (newCards as any)[index - 1]
    } else {
      newCards[index] = newCards[index + 1]! // Correction: (newCards as any)[index]
      newCards[index + 1] = card! // Correction: (newCards as any)[index + 1]
    }
              ...
              <Droppable droppableId="dashboard-cards">
                {(provided) => (
                  <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2">
                    {cards.map((card, index) => ( // Correction: _index en index
