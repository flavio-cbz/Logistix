...
export interface InteractiveChartProps {
  ...
  colors?: string[]
  onDataPointClick?: (point: ChartDataPoint, _index: number) => void
  onDataPointHover?: (point: ChartDataPoint | null, _index: number) => void
...
export const InteractiveChart: React.FC<InteractiveChartProps> = ({
  ...
  responsive = true,
  theme: _theme = 'auto',
  colors = ['hsl(var(--primary))', 'hsl(var(--destructive))', 'hsl(var(--success))', 'hsl(var(--warning))', 'hsl(var(--accent))'],
  onDataPointClick,
  onDataPointHover,
  onSelectionChange,
  className
  ...
  const combinedRef = useCallback((node: HTMLDivElement | null) => {
    ...
    if (accessibility.keyboardNavigation) {
      (keyboardRef as React.MutableRefObject<HTMLDivElement | null>).current = node
    }
  ...
  const xScale = useCallback(() => {
    ...
    if (xValues.length === 0) return { min: 0, max: 1, range: 1 }
    const xMin = Math.min(...xValues)
    const xMax = Math.max(...xValues)
    const range = xMax - xMin || 1
    return { min: xMin, max: xMax, range }
  }, [data])

  const yScale = useCallback(() => {
    const yValues = data.map(d => d.y)
    if (yValues.length === 0) return { min: 0, max: 1, range: 1 }
  ...
  const getXPosition = useCallback((value: number) => {
    const scale = xScale()
    const r = scale.range || 1
    return ((value - scale.min) / r) * chartWidth
  }, [xScale, chartWidth])

  const getYPosition = useCallback((value: number) => {
    const scale = yScale()
    const r = scale.range || 1
    return chartHeight - ((value - scale.min) / r) * chartHeight
  }, [yScale, chartHeight])

  // Gestionnaires d'événements
  const handlePointClick = useCallback((point: ChartDataPoint, _index: number) => {
    setSelectedPoints(prev => {
      const newSelection = prev.includes(_index)
        ? prev.filter(i => i !== _index)
        : [...prev, _index]
  ...
  const handlePointHover = useCallback((point: ChartDataPoint | null, _index: number) => {
    setHoveredPoint(point ? _index : null)
    onDataPointHover?.(point, _index)
  ...
  const renderScatterPlot = () => {
    return data.map((point, _index) => {
      const x = getXPosition(typeof point.x === 'number' ? point.x : 0)
      const y = getYPosition(point.y)
      const isSelected = selectedPoints.includes(_index)
      const isHovered = hoveredPoint === _index
      ...
      return (
        <circle
          ...
          onClick={() => handlePointClick(point, _index)}
          onMouseEnter={() => interactions.hover && handlePointHover(point, _index)}
          onMouseLeave={() => interactions.hover && handlePointHover(null, _index)}
          role="button"
  ...
  const renderBarChart = () => {
    ...
    return data.map((point, _index) => {
      const x = (_index * chartWidth / data.length) + barSpacing / 2
      const y = getYPosition(point.y)
  ...
  const renderLineChart = () => {
    const points = data.map((point, _index) => {
      const x = getXPosition(typeof point.x === 'number' ? point.x : _index)
      const y = getYPosition(point.y)
        ...
        <table className="w-full border-collapse border border-[hsl(var(--border))]">
          ...
          <thead>
            <tr>
              ...
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Y</th>
              {data[0]?.label && (
                <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Label</th>
              )}
            </tr>
          </thead>
          <tbody>
            {data.map((point, _index) => (
              <tr
                key={_index}
                className={selectedPoints.includes(_index) ? 'bg-[hsl(var(--primary))]' : ''}
              >
                <td className="border border-[hsl(var(--border))] px-2 py-1">{point.x}</td>
                <td className="border border-[hsl(var(--border))] px-2 py-1">{point.y}</td>
                {point.label && (
                  <td className="border border-[hsl(var(--border))] px-2 py-1">{point.label}</td>
                )}
              </tr>
            ))}
          </tbody>
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        className="border border-[hsl(var(--border))] bg-[hsl(var(--card))]"
        role="presentation"
        aria-hidden="true"
      >
        {renderAxes()}
      ...
      {hoveredPoint !== null && interactions.tooltip && (
        <div
          ...
          style={{
            left: getXPosition(typeof data[hoveredPoint]?.x === 'number' ? data[hoveredPoint]?.x as number : 0) + margin.left,
            top: getYPosition(data[hoveredPoint]?.y ?? 0) + margin.top - 30
          }}
        >
