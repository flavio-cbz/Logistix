...
import type { AdvancedMetrics, ComparisonResult } from '@/lib/analytics/advanced-analytics-engine'
...
export const ComparisonChart: React.FC<ComparisonChartProps> = ({
  ...
  const colors = [
    'hsl(var(--primary))', 'hsl(var(--destructive))', 'hsl(var(--success))', 'hsl(var(--warning))', 'hsl(var(--accent))',
    'hsl(var(--info))', '#f97316', '#84cc16', '#ec4899', '#6366f1'
  ]
          ...
          {Array.from({ length: maxBins }).map((_, binIndex) => {
            ...
            return (
              <rect
                ...
                width={width * 0.8}
                height={height}
                fill={difference >= 0 ? 'hsl(var(--success))' : 'hsl(var(--destructive))'}
                opacity={isSelected ? 1 : 0.7}
                stroke={isSelected ? '#000' : 'none'}
                strokeWidth={isSelected ? 1 : 0}
                style={{ cursor: 'pointer' }}
  ...
  const renderStatistics = () => {
    ...
    return (
      <div className="absolute top-4 right-4 bg-[hsl(var(--card))] border rounded-lg p-4 shadow-lg max-w-xs">
        <h4 className="font-medium mb-2">Statistiques de comparaison</h4>

        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span>Différence de prix:</span>
            <span className={comparison.metrics.priceDifference.percentage >= 0 ? 'text-[hsl(var(--success-foreground))]' : 'text-[hsl(var(--destructive-foreground))]'}>
          ...
          <div className="flex justify-between">
            ...
            <span className={comparison.metrics.volumeDifference.percentage >= 0 ? 'text-[hsl(var(--success-foreground))]' : 'text-[hsl(var(--destructive-foreground))]'}>
        ...
        {comparison.insights.length > 0 && (
          <div className="mt-3 pt-3 border-t">
            ...
            <ul className="text-xs space-y-1">
              {comparison.insights?.slice(0, 3).map((insight: any, insightIndex: number) => (
                <li key={insightIndex} className="text-[hsl(var(--muted-foreground))]">• {insight}</li>
              ))}
            </ul>
          </div>
        )}
    ...
    return (
      <div className="mt-4 overflow-auto">
        <table className="w-full border-collapse border border-[hsl(var(--border))]">
          ...
          <thead>
            <tr>
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Comparaison</th>
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Diff. Prix (%)</th>
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Diff. Volume (%)</th>
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Similarité (%)</th>
              <th className="border border-[hsl(var(--border))] px-2 py-1 bg-[hsl(var(--muted))]">Signification (%)</th>
            </tr>
          </thead>
          <tbody>
            {comparisons.map((comparison, index) => (
              <tr
                key={index}
                className={selectedComparison === index ? 'bg-[hsl(var(--primary))]' : ''}
              >
    ...
    >
      ...
      <div className="relative">
        <svg
          width={width}
          height={height}
          className="border border-[hsl(var(--border))] bg-[hsl(var(--card))] rounded"
          role="presentation"
          aria-hidden="true"
        >
          {renderAxes()}
