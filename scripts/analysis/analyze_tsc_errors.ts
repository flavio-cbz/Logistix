...
function main() {
  try {
    ...
    console.log(`Analyse terminée. Rapport sauvegardé dans ${OUTPUT_REPORT_FILE}`);
    console.log(`Total d'erreurs: ${report.totalErrors}`);
    console.log(`Total de fichiers affectés: ${report.totalFilesAffected}`);
    console.log('\nTop 10 des fichiers les plus problématiques:');
    report.top10Files.forEach((file, index) => {
      console.log(`${index + 1}. ${file.filePath}: ${file.totalErrors} erreurs`);
      console.log('   Types d\'erreurs fréquents dans ce fichier:');
      const sortedErrorTypes = Object.entries(file.errorTypes).sort((a, b) => b[1] - a[1]);
      sortedErrorTypes.slice(0, 3).forEach(([code, count]) => {
        console.log(`     - ${code}: ${count} fois`);
      });
    });
    console.log('\nTypes d\'erreurs les plus fréquents globalement:');
    Object.entries(report.mostFrequentErrorTypesGlobal).slice(0, 5).forEach(([code, count]) => {
      console.log(`- ${code}: ${count} fois`);
    });
