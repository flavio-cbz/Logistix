...
def find_representative_item(brand_id: int, catalog_id: int, status_id: int, access_token: str) -> Optional[int]:
    """
    ...
    Les données proviennent exclusivement de l'API Vinted.
    """
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 5:
        print("Usage: python find_representative_item.py <brand_id> <catalog_id> <status_id> <access_token>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        item_id = find_representative_item(brand_id, catalog_id, status_id, access_token)
