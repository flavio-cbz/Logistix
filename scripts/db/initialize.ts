...
async function runMigrations(): Promise<void> {
  ...
  for (const file of migrationFiles) {
    ...
    // 1. Supprimer les markers de breakpoint Drizzle pour éviter qu'ils ne soient traités comme des commentaires
    const sqlContentNoBreakpoints = sqlContent.replace(/--> statement-breakpoint/g, '');

    // 2. Supprimer les commentaires et lignes vides
    const cleanedContent = sqlContentNoBreakpoints
      .replace(/\/\*[\s\S]*?\*\//g, '')
      .split('\n')
      .filter((line: string) => {
