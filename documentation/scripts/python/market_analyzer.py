...
def analyze_market_by_product_name(search_text: str, access_token: str, is_test_mode: bool = False) -> dict:
    """
    ...
    if is_test_mode:
        ...
        test_data = {
            ...
            "priceAnalysis": {"min": 15.0, "max": 60.0, "average": 35.0, "median": 30.0},
            "summary": {"itemsFound": 50, "sellersCount": 25},
            "brandDistribution": {"Test Brand": 50},
            "conditionDistribution": {"Like New": 50}
        }
        # Sauvegarder l'analyse de test
        save_analysis(search_text, test_data)
        logger.info(f"Market analysis (test mode) saved for: {search_text}")
        return test_data
