...
import { MarketAnalysisRequestSchema, MarketAnalysisRequest } from '@/lib/validations/vinted-market-analysis-schemas'
...
function normalizeBrandOptions(input: unknown): BrandOption[] {
  ...
  for (const item of input) {
    if (isRecord(item)) {
      ...
      const titleRaw = typeof item.title === "string" ? item.title : name ?? ""
...
export default function AnalysisForm({ onSubmit, isLoading, error, initialValues, onReset }: any): JSX.Element {
  ...
  const hasValidInput = Boolean(productName && productName.length >= MIN_PRODUCT_NAME_LEN)
  const hasCategory = typeof catalogId === "number" && Number.isFinite(catalogId)
  ...
  useEffect(() => {
    if (!hasValidInput || !hasCategory) {
      form.setValue("brandId", undefined, { shouldDirty: true, shouldValidate: false })
    }
  ...
  useEffect(() => {
    let active = true
    const run = async () => {
      if (productName && productName.length >= MIN_PRODUCT_NAME_LEN && catalogId) {
        const currentId = ++brandQueryIdRef.current
        try {
          const suggested = await getBrandSuggestions(productName, catalogId)
  ...
  const handleReset = useCallback(() => {
    ...
    if (typeof initialValues?.catalogId !== "undefined") resetValues.catalogId = initialValues.catalogId
    if (typeof initialValues?.brandId !== "undefined") resetValues.brandId = initialValues.brandId
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {!categoryLoading && !categoryError && (categorySuggestions?.length ?? 0) > 0 && (
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <CatalogSelector
                        value={field.value}
                        onValueChange={(categoryId: number, category: { id: number; title: string }) => {
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={typeof field.value === "number" && Number.isFinite(field.value) ? field.value : 100}
                          onChange={(event) => {
                            ...
                            const clamped = Math.max(MAX_PRODUCTS_MIN, Math.min(MAX_PRODUCTS_MAX, Math.trunc(raw)))
                      ...
                      <FormControl>
                        <div className="flex flex-col gap-2">
                          {ITEM_STATE_OPTIONS.map((option) => {
                            ...
                            const isChecked = Array.isArray(field.value) ? field.value.map(String).includes(selectedId) : false
                            return (
                              <label key={option.id} className="flex items-center gap-2">
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => {
                                    if (checked) {
                                      const next = new Set<string>(Array.isArray(field.value) ? field.value.map(String) : [])
                                      next.add(selectedId)
            ...
            {hasValidInput && !hasCategory && (
              <Alert role="status" aria-live="polite">
                <AlertCircle className="h-4 w-4" /> {/* Changed from Info to AlertCircle */}
                <AlertDescription>
