...
export default function MarketAnalysisChart({
  ...
  const generateChartData = () => {
    ...
    completedHistorical.forEach((_item) => {
      data.push({
        date: new Date(_item.createdAt).toLocaleDateString("fr-FR", {
        ...
        <div className="bg-background border rounded-lg p-3 shadow-md">
          ...
          {payload.map((entry: any, _index: number) => (
            <p key={_index} style={{ color: entry.color }}>
              {" "}
              {/* Corrected index to _index */}
              {entry.name}:{" "}
              {entry.name === "Prix" ? `${entry.value} €` : entry.value}
            </p>
          ))}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Historique des prix et volume des ventes pour les analyses de ce
          produit
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {data.some((d) => d.type === "simulated") && (
          <div className="mt-4 p-3 bg-muted/50 rounded-lg">
            <p className="text-xs text-muted-foreground">
              * Les données simulées sont générées pour fournir un contexte
              visuel. Effectuez plus d'analyses pour obtenir un historique réel.
            </p>
          </div>
        )}
      </CardContent>
