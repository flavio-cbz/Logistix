...
const activityLabel = (vol: number): "Élevée" | "Moyenne" | "Faible" =>
  vol >= HIGH_ACTIVITY_MIN ? "Élevée" : vol >= MEDIUM_ACTIVITY_MIN ? "Moyenne" : "Faible"
...
export default function SalesVolumeChart({ analysis }: SalesVolumeChartProps) {
  ...
  const minPrice = toNumber((analysis as any)?.priceRange?.min, avgPrice > 0 ? Math.max(0, avgPrice - 10) : 0)
  const maxPrice = toNumber((analysis as any)?.priceRange?.max, avgPrice > 0 ? avgPrice + 10 : minPrice + 10)
  ...
  const priceDistribution: PriceBucket[] = useMemo(() => {
    ...
    const buckets: PriceBucket[] = Array.from({ length: numBuckets }).map((_, i) => {
      ...
      const variation = Math.floor(baseCount * (1 - normalizedDistance) * (0.5 + rng()))
      const count = salesVolume === 0 ? 0 : Math.max(0, baseCount + variation)
    ...
    if (total > 0) {
      buckets.forEach((b, idx) => {
        ...
        if (idx === buckets.length - 1) {
          const sumPct = buckets.slice(0, -1).reduce((s, x) => s + x.percentage, 0)
  ...
  const sizeDistribution: SizeSlice[] = useMemo(() => {
    ...
    const palette: string[] = ["#8884d8", "#82ca9d", "#ffc658", "#ff7300", "#22c55e", "#a855f7", "#06b6d4"]

    const raw: SizeSlice[] = sizes.map((name, _index) => {
      const base = Math.max(0, Math.floor(salesVolume / 3))
      const factor = 0.3 + rng()
      const value = salesVolume === 0 ? 0 : Math.floor(base * factor * (0.4 + rng()))
      return {
        name,
        value,
  ...
  const PieTooltip = memo(function PieTooltipMemo({
    ...
    const percent = salesVolume > 0 ? Math.round((value / salesVolume) * 100) : 0
    return (
      <div className="bg-background border rounded-lg p-3 shadow-md">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card aria-labelledby="volume-total-title" aria-describedby="volume-total-desc">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle id="volume-total-title" className="text-sm font-medium">
              Volume total
            </CardTitle>
            <ShoppingCart aria-hidden className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
        ...
        <Card aria-labelledby="concurrence-title" aria-describedby="concurrence-desc">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card aria-labelledby="diversite-title" aria-describedby="diversite-desc">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle id={`${priceChartId}-title`}>Distribution des Ventes par Tranche de Prix</CardTitle>
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={priceDistribution}>
                ...
                <Bar dataKey="count" name="Ventes" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      {sizeDistribution.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle id={`${sizeChartId}-title`}>Distribution par Taille (Estimation)</CardTitle>
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                {sizeDistribution.map((_item) => (
                  <div key={_item.name} className="flex items-center justify-between"> {/* Corrected item to _item */}
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: _item.color }} /> {/* Corrected item to _item */}
                      <span className="text-sm">{_item.name}</span> {/* Corrected item to _item */}
                    </div>
                    <div className="text-sm text-muted-foreground">{_item.value} ventes</div> {/* Corrected item to _item */}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground">
                * Les données de taille sont estimées car elles ne sont pas toujours disponibles dans les données Vinted.
                Cette répartition est basée sur des tendances générales.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card aria-labelledby={`${insightsId}-title`}>
        <CardHeader>
          <CardTitle id={`${insightsId}-title`}>Analyse du Volume de Ventes</CardTitle>
        </CardHeader>
        <CardContent>
