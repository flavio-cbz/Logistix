...
const MetricCard = memo(function MetricCard({
  ...
  const renderedIcon =
    isValidElement(icon) ? cloneElement(icon as ReactElement<any, string | React.JSXElementConstructor<any>>, { "aria-hidden": true, focusable: false } as Partial<Attributes>) : null
  ...
  return (
    <Card role="region" aria-labelledby={titleId} data-testid={testId}>
      ...
      <CardContent>
        <div className={cx("text-2xl font-bold", valueClassName)} aria-live="polite" data-testid={testId ? `${testId}-value` : undefined}>
          {value}
        </div>
 ...
 */
function KeyMetricsWidgetBase({ analysis }: KeyMetricsWidgetProps): ReactElement {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <MetricCard
        ...
        icon={<TrendingDown className="h-4 w-4 text-[hsl(var(--success-foreground))]" />}
      ...
      <MetricCard
        ...
        icon={<TrendingUp className="h-4 w-4 text-[hsl(var(--destructive-foreground))]" />}
 ...
 */
const areEqual = (prev: KeyMetricsWidgetProps, _next: KeyMetricsWidgetProps) => {
