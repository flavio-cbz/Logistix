...
interface CatalogSelectorProps {
  ...
  readonly onValueChange: (categoryId: number, category: VintedCategory) => void
  readonly placeholder?: string
  readonly disabled?: boolean
}

// Utils purs, hors composant (pas de dépendance à l'état React)
function findCategoryById(categories: ReadonlyArrayDeep<VintedCategory>, id: number): VintedCategory | null {
...
export function CatalogSelector({
  ...
  const [allCategories, setAllCategories] = useState<ReadonlyArrayDeep<VintedCategory>>([])
  const [currentCategories, setCurrentCategories] = useState<ReadonlyArrayDeep<VintedCategory>>([])
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        console.error("[CatalogSelector] Échec du chargement des catégories:", err)
      } finally {
  ...
  const handleSelect = useCallback(
    (cat: VintedCategory) => {
      if (cat.catalogs && cat.catalogs.length > 0) {
        setHistory((prev: ReadonlyArrayDeep<VintedCategory>[]) => [...prev, currentCategories])
  ...
  const hasChildren = (c: VintedCategory) => Boolean(c.catalogs && c.catalogs.length > 0)
              ...
              {filtered.map((cat) => (
                ...
                >
                  ...
                  {hasChildren(cat) ? (
                    ...
                    <Badge variant="outline" className="text-2xs">ID: {cat.id}</Badge>
                  )}
                </CommandItem>
              ))}
            </CommandGroup>

            <span id={`${listboxId}-status`} className="sr-only" aria-live="polite">
              {isLoading ? "Chargement des catégories" : `${filtered.length} résultat(s)`}
            </span>
          </CommandList>
        </Command>
      )}
