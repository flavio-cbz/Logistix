...
export function AiModelSelector({
    ...
    async function loadConfig() {
      ...
      try {
        ...
        if (data && data.ai_config) {
          const config =
            typeof data.ai_config === "string"
              ? JSON.parse(data.ai_config)
              : data.ai_config;
  ...
  const discoverModels = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data.error || "Erreur lors de la récupération des modèles",
        );
      ...
      setError(
        "Échec de la requête : " +
          (e.message || "Impossible de découvrir les modèles."),
      );
    } finally {
      setLoading(false);
    }
